...
files.each do |file|
  ...
  artist = Artist.new #unless Artist.all.any?{|artist| artist.name == artist_name}
  artist.name = artist_name

  song_parse = (file.split(" - ")[1]) 
  song_name = song_parse.split(" [")[0] unless song_parse.nil?
  song = Song.new
  song.name = song_name
  


  genre_parse = song_parse.slice(song_parse.index("[") +1 ... song_parse.index("]")) unless song_parse.nil?
  genre = Genre.new
  genre.name = genre_parse
